...
#ifndef LLAMA_SERVER_VERBOSE
#define LLAMA_SERVER_VERBOSE 1
#endif
...
struct llama_server_context {
  ...
  float prompt_processing_progress = 0.0;
  ...
  void reset_state() {
    ...
    prompt_processing_progress = 0.0;
  ...
  void unload_model() {
    std::cout << "LOG: llama.unload_model()"
              << "\n";
    // llama_clear_kv_cache(model);
    llama_free_model(model);
    model = nullptr;
    llama_free(ctx);
    ctx = nullptr;
  ...
  completion_token_output nextToken() {
    ...
    while (n_past < embd.size()) {
      ...
      if (async_compl_cancel_requested == 1 ||
          llama_decode(ctx,
      ...
      n_past += n_eval;
      prompt_processing_progress = (float)n_past / (float)embd.size();
    }
...
#else
      LOG_WARNING("llama.cpp was compiled without cuBLAS. It is not possible "
...
#else
      LOG_WARNING("warning: llama.cpp was compiled without cuBLAS. Disabling "
...
void async_completion_worker() {
  ...
  while (llama.has_next_token && async_compl_cancel_requested != 1) {
    ...
    if (async_slack_ms > 0) {
      std::this_thread::sleep_for(std::chrono::milliseconds(async_slack_ms));
    }
    ...
    if (stop_pos == std::string::npos ||
      ...
      // const std::string str = data.dump(-1, ' ', false,
      // json::error_handler_t::replace)

      LOG_VERBOSE("data stream",
                  {{"to_send", data.dump(-1, ' ', false,
                                         json::error_handler_t::replace)}});
...
const char *async_completion_init(const char *req_json) {
  ...
  log_enable();
  LOG("HELLO WORLD FROM LLAMARPC LOGS");
  ...
  llama_reset_timings(llama.ctx);
  auto req = json::parse(req_json);
  auto it = req.find("async_slack_ms");
  if (it != req.end()) {
    it->get_to(async_slack_ms);
    if (async_slack_ms) {
      LOG("async_slack_ms = %d", async_slack_ms);
    }
  }

  parse_options_completion(req, llama);
...
const char *async_completion_poll(const char *cmd_json) {
  ...
  ret["prompt_processing_progress"] = llama.prompt_processing_progress;
...
void from_json(const json &j, llama_sampling_params &p) {
  ...
  if (j.find("repeat_penalty") != j.end())
    j.at("repeat_penalty").get_to(p.repeat_penalty);
  if (j.find("repeat_last_n") != j.end())
    j.at("repeat_last_n").get_to(p.repeat_last_n);
  if (j.find("frequency_penalty") != j.end())
    j.at("frequency_penalty").get_to(p.frequency_penalty);
  if (j.find("presence_penalty") != j.end())
    j.at("presence_penalty").get_to(p.presence_penalty);
...
void from_json(const json &j, gpt_params &p) {
  ...
  if (j.find("tensor_split") != j.end()) {
    for (size_t i = 0; i < LLAMA_MAX_DEVICES; ++i) {
      if (j["tensor_split"].size() > i) {
        j["tensor_split"][i].get_to(p.tensor_split[i]);
      }
    }
  }
  ...
  // Deserialize the nested struct llama_sampling_params
  if (j.find("sampling_params") != j.end()) {
    from_json(j["sampling_params"], p.sampling_params);
  }

  // ... continue for all fields in the struct
  ...
  if (j.find("grammar") != j.end())
    j.at("grammar").get_to(p.grammar);
...
void server_params_parse_from_json(const char *json_string, gpt_params &sparams,
...
const char *poll_system_status() {
  ...
  j["loading_progress"] = get_current_model_load_progress();

  return strdup(j.dump(-1, ' ', false, json::error_handler_t::replace).c_str());
}

// const char* save_state(const char* req_json) {
//   bool success = llama_load_session_file(llama.ctx, const char *
//   path_session, llama_token * tokens_out, size_t n_token_capacity, size_t *
//   n_token_count_out)
// }
...
// const char *json_rpc(const char *method, const char *path, const char
// *headers,
//                      const char *body) {
//   printf("DEBUG: %s \n", headers);
//   // return svr.call_json_request(method, path, headers, body);
// }

void deinit() {
  LOG("LIBRPCSERVER: deinit()");
  ...
  // llama_backend_free();
  // llama_backend_init_done = false;

  reset_async_state();

  // llama =std::move(llama_server_context());

  LOG("LIBRPCSERVER: deinit() DONE");
}
...
/*
  ...
  svr.Get("/", [](const Request &, Response &res) {
    res.set_content(reinterpret_cast<const char *>(&index_html), index_html_len,
                    "text/html");
    return false;
  });
  ...
  svr.Get("/index.js", [](const Request &, Response &res) {
    res.set_content(reinterpret_cast<const char *>(&index_js), index_js_len,
                    "text/javascript");
    return false;
  });
  ...
  svr.Get("/completion.js", [](const Request &, Response &res) {
    res.set_content(reinterpret_cast<const char *>(&completion_js),
                    completion_js_len, "application/javascript");
    return false;
  });

  // this is only called if no index.html is found in the public --path
  svr.Get("/json-schema-to-grammar.mjs", [](const Request &, Response &res) {
    res.set_content(reinterpret_cast<const char *>(&json_schema_to_grammar_mjs),
                    json_schema_to_grammar_mjs_len, "application/javascript");
    return false;
  });
  ...
  svr.Post("/completion", [](const Request &req, Response &res) {
    ...
    if (!llama.stream) {
      ...
      const json data =
          format_final_response(llama, llama.generated_text, probs);
      ...
      const auto chunked_content_provider = [&](size_t, DataSink &sink) {ç
        size_t sent_count = 0;
        size_t sent_token_probs_index = 0;
  ...
  svr.Post("/infill", [](const Request &req, Response &res) {
    auto lock = llama.lock();
  ...
  svr.Get("/model.json", [](const Request &, Response &res) {
    const json data = format_generation_settings(llama);
    return res.set_content(data.dump(-1, ' ', false,
  json::error_handler_t::replace), "application/json");
  });
  ...
  svr.Post("/tokenize", [](const Request &req, Response &res) {
    auto lock = llama.lock();
  ...
  svr.Post("/detokenize", [](const Request &req, Response &res) {
    auto lock = llama.lock();
  ...
  svr.Post("/embedding", [](const Request &req, Response &res) {
    auto lock = llama.lock();
