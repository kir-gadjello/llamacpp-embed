...
struct server_context {
    ...
    void reset_state() {
        ...
        if (mutex_results.try_lock()) {
            mutex_results.unlock();
        }
        if (mutex_tasks.try_lock()) {
            mutex_tasks.unlock();
        }
    }
...
void setup_handlers(llama_server_context & llama, gpt_params & params, server_params & sparams, httplib::Server & svr) {
...
int main(int argc, char **argv)
{
    ...
    // setup_handlers(llama, params, sparams, svr);
...
const char *poll_system_status() {
  ...
  j["loading_progress"] = 0.0;  // TODO get_current_model_load_progress();
